...
cd src && uv run python main.py play --game tictactoe
cd src && uv run python main.py play --game connectfour

# Train Q-learning agent
cd src && uv run python main.py train --game tictactoe --episodes 50000 --save
cd src && uv run python main.py train --game connectfour --episodes 50000 --save

# Headless evaluation (no pygame)
cd src && uv run python main.py eval --game tictactoe --agent1 ql --agent2 minimax --runs 500
...
Two fully separate game packages (`tictactoe/`, `connectfour/`) with intentional code duplication. All source lives in `src/`.
...
- `minimax.py` — alpha-beta minimax. TicTacToe uses `minimax_alpha_beta` (returns a `MinimaxResult`). ConnectFour uses `minimax` with `non_terminal_score()` heuristic that scores 4-cell sliding windows.
...
- `TicTacToe.move(x, y, player)` — takes explicit row/col coordinates; `actions()` returns `[(x, y)]` tuples; agent code must unpack with `*move`.
